...
**Product_Series/Product_Type(신규 Key, 사용자 지시 2026-07-17, 2026-07-18 재정의)**: A10_AE_EN.docx 최상단 헤더("A10 - Medium throw line source" / "A Series" / "Medium throw: 35 m" / "Deployment in horizontal / vertical lines" / "Incremental coverage with **fixed** inter-element angles")에서 채택. Product_Type은 최초 헤더 1번째 줄("Medium throw line source", A/K 시리즈 공통으로 구분력이 없는 표현)이 아니라 5번째 줄의 "fixed inter-element angles"(고정 인터엘리먼트 각도 = 곡률이 배치 시점에 확정되는 constant curvature 아키텍처)를 근거로 "Constant Curvature Line Source"로 재정의(사용자 지적 2026-07-18) — K 시리즈의 "variable inter-element angles"(가변 각도 = variable curvature)와 대비되는 L-Acoustics 자신의 실제 구분축.
...
**Cardioid_Capability=No(확정적 비존재)**: A10_OM_EN_2.1.pdf/A10_AE_EN.docx 전량을 "cardioid" 키워드로 스캔 — A10 Focus 자신에 대한 언급 0건, 검출된 언급은 전부 컴패니언 서브우퍼의 일반 반전 배치 프리셋 안내뿐(4대 어레이 중 1대 반전 방식, 원문 "an array of 4 subwoofers" — TODO.md/사용자 확인 완료).
...
**비적용**: 없음 — 0건. (PA_COM_Pinout_*/Terminal_Block_Pinout_* 등 타 커넥터 고유 Key는 A10 Focus 자신의 SpeakON_Pinout_*으로 이미 표현되어 애초에 생성하지 않음, null 목록에서 제외.)
...
| v1.4 | Cardioid_Capability 신규 Key 적용(No, 원문 전량 스캔 근거, TaskList #39). d&b 고유 signal_processing(CUT_Mode/HFC/Coupling) 섹션 삭제(사용자 지적 2026-07-18, `speakers/CLAUDE.md` 규칙 2 위반 시정). Product_Series/Product_Type 신규 Key 적용 — Series="A Series", Type은 최초 "Medium throw line source"(AE 헤더 1번째 줄)로 채웠다가 사용자 지적(2026-07-18)에 따라 AE 헤더 5번째 줄("fixed inter-element angles")을 근거로 "Constant Curvature Line Source"로 재정의(K 시리즈의 "variable inter-element angles"와 대비되는 실제 구분축). |
| v1.4 to v1.5 | (표기 정정, Minor) 각주 버전 참조 최신성 감사(2026-07-18)에서 preset_guide_and_matching의 null 셀이 "Minimum_Line_Length 전 행"이라는 서술로 뭉뚱그려져 실제로는 3개 Configuration 행에 걸친 5개 개별 null 셀(단독/단일 엘리먼트 2행은 Recommended_Ratio·Minimum_Line_Length 둘 다, KS21 조합 1행은 Minimum_Line_Length만)인데도 총계 산정 시 1건으로만 반영되어 있던 결합형 불릿 오카운트를 발견·정정(K1_v1.11.md/L2_v1.7.md와 동일 유형). 미확인 11건→15건, 총계 11건→15건으로 재계산. 데이터(Key/Value/Unit) 자체는 전혀 변경하지 않았다. |
