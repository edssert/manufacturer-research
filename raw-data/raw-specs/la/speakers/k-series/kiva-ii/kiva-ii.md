...
**[MR_v1.8 반영 메모]**: 이 파일은 `upload/Kiva_II_v1.7.md`(별도 audio-spec-parsing 프로젝트 산출물)를 이 저장소의 raw-spec으로 그대로 이관한 것이다. `public/js/domains/speakers/data/la/k-series.data.js`의 Kiva II 엔트리에 Cardioid_Capability/Safety_Factor/Max_Wind_Load를 반영했다(speakON 커넥터라 PA-COM Pinout은 미해당; WEEE_Marking은 이 파일에서도 미확인). 기존 필드는 대조 후 이상 없음(Width/Height/Depth 525/202/357 일치, watt=186이 RMS_Power_Handling_Overall과 일치).
