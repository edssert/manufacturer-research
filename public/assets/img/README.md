...
스펙 데이터 폴더(`public/js/domains/{도메인}/data/{mk}/{series}.data.js`)와
1:1 대응되도록 `public/assets/img/{도메인}/{mk}/{series-slug}/파일명.{png|webp|jpg}`
구조로 정리합니다.

```
public/assets/img/
  speakers/                  ← public/js/domains/speakers/data/ 에 대응
    la/                      ← mk: L-Acoustics
    ...
    db/                       ← mk: d&b audiotechnik
      ...
      cl/
  amplifiers/                 ← public/js/domains/amplifiers/data/ 에 대응
  dsps/                       ← DSP 이미지 추가 시
...
data.js 안의 `img` 필드는 저장소/배포 루트 기준 상대경로를 그대로 담습니다.
예: `"img": "public/assets/img/speakers/la/k-series/spk-la-k1.png"`.

## 새 이미지 추가 체크리스트

1. **대응하는 하위 폴더에 저장한다.** 새 스피커를 `public/js/domains/speakers/data/la/k-series.data.js`에
...
4. **여백을 정규화한다.** 아래 스크립트로 전체 이미지(하위 폴더 재귀 포함)의
   상하좌우 여백을 동일 비율(기본 8%)로 통일합니다. `dry`는 원본과 예상
   출력의 바이트 크기·해상도·알파 여부·SHA-256을 출력할 뿐 파일을 바꾸지
   않습니다. 결과를 검수하기 전에는 `apply`를 실행하지 않습니다.

   ```bash
   # 스크립트는 scripts/ 에 있고, 대상 폴더(public/assets/img)는 스크립트가
   # 스스로 찾는다 — 어느 위치에서 실행해도 된다.
   pip install Pillow --break-system-packages          # 최초 1회만
   python3 scripts/normalize_images.py dry \
     --manifest image-normalize-plan.json              # 변경 없는 분석
   cp -r public/assets/img /path/to/backup/img_backup  # apply 전 백업
   python3 scripts/normalize_images.py apply \
     --approved-manifest image-normalize-plan.json \
     --manifest image-normalize-applied.json           # 검수 후 명시 실행
   ```

   `apply`는 검토한 dry-run manifest와 현재 원본 해시·예상 출력이 모두 같을
...
`{도메인접두어}-{제조사코드}-{모델슬러그}.{확장자}` 형식을 따릅니다.
확장자는 실제 인코딩과 일치하는 `.png`, `.webp`, `.jpg`/`.jpeg` 중 하나를
사용합니다. 예: `spk-la-k1.png` (스피커 · L-Acoustics · K1),
...
1. `public/js/domains/{도메인}/data/{mk}/{series}.data.js`에 데이터를 추가한다.
2. 동일한 경로 규칙으로 `public/assets/img/{도메인}/{mk}/{series-slug}/`
   폴더를 만들고 이미지를 넣는다.
...
PNG, JPEG, WebP가 기본 지원됩니다. 다른 형식을 추가하려면 두 스크립트의
지원 확장자와 인코딩 정책을 함께 갱신해야 합니다. 폴더 구조 자체가
"스펙 데이터 폴더와 1:1 대응"이라는
