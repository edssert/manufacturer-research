...
const stylesheetHrefs = [...indexHTML.matchAll(/<link\s+rel="stylesheet"\s+href="([^"]+)"/g)]
  .map(match => match[1]);
...
check(
  "기본 표 스타일 뒤에 Split View 보정 스타일을 로드함",
  specIndex !== -1 && splitIndex > specIndex,
);
...
check(
  "사용되지 않는 3열 변경자 계약이 남아 있지 않음",
  !specCSS.includes("match-table--cols-3"),
);

const mobileRules = leafRules(blockFor(specCSS, "@media (max-width: 480px)"));
const speakerMinWidth = mobileRules.find(
  rule => rule.selector === ".match-table--toggleable .match-table__row",
);
const ampMinWidth = mobileRules.find(
  rule => rule.selector === ".match-table--amp-view .match-table__row",
);
check(
  "모바일 Speaker 6열 표가 520px 최소폭을 가짐",
  speakerMinWidth?.declarations.includes("min-width: 520px;"),
);
check(
