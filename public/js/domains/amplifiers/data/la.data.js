...
// 모든 앰프는 브랜드와 무관하게 같은 필드 집합과 중첩 구조를 사용해야
// 모달·카드 양식이 일치한다. 데이터가 없는 필드는 null 로 채우고,
// amplifiers.view.js 의 specRow 는 값이 없으면 "—"로 표시하도록 되어 있어
// 화면에는 항상 같은 행/섹션 구성이 보인다.
// 외부 제어 연동은 ecosystem.integrations 하나로 관리한다. gpio 는 독립
// 정보로 유지하고, 근거가 없는 dimensions는 null로 둔다.
export const LA_AMPLIFIERS = [
  {
    ...
    "output": {
      ...
      "minImpedance": "4 ohms",
      // [출력 합계] 채널당 정격은 16x80W와 16x160W로 조건에 따라 달라
      // 단일 총출력으로 확정할 수 없다. powerTotal은 null로 두고 카드와
      // 게이지는 "N x W" 채널당 표기만 사용한다.
      "powerTotal": null,
  ...
  {
    // LA2Xi도 다른 앰프와 같은 필드 구조를 따른다. BTL/PBTL 출력처럼 일반
  ...
  },
  {
    ...
    // 개별 앰프와 구분하기 위해 앰프 탭의 "Racks"로 분류한다. 랙 고유 구조
    ...
    "rack": {
      ...
      // 페이지의 "System Elements" 목록을 앱의 액세서리 ID로 매핑한다
      // (리깅 3 + 케이블 다수 — 기준 자료:
  ...
  },
  {
    ...
    "rack": {
      ...
      // 페이지의 "System Elements" 목록 25개를 앱의 액세서리 ID로 매핑한다
      // (기준 자료: raw-data/raw-specs/la/amplifiers/
      ...
      "relatedAccessoryIds": ["acc-la-la-rak-bump-iii", "acc-la-la-rakmount", "acc-la-k2-rakmount", "acc-la-pow2", "acc-la-doe", "acc-la-dom-cross", "acc-la-domp-2", "acc-la-domf", "acc-la-domm", "acc-la-sp", "acc-la-sp-y1", "acc-la-do", "acc-la-dofill-la8", "acc-la-do3wfill", "acc-la-dosub-la8"]
    }
  }
];
