 ...
 * 섹션으로 부드럽게 이동한다.
 *
 * IntersectionObserver 를 쓰지 않는 이유: 그건 "경계를 넘었다"는 이산 신호만
 * 주는데, 표시가 스크롤에 맞춰 연속으로 미끄러지려면 "섹션 사이 어디쯤인가"를
 * 소수로 알아야 한다. 그래서 스크롤 이벤트를 rAF 로 묶어 직접 계산한다.
 *
 * 성능은 캐시로 해결한다 — 섹션들의 문서상 y 좌표는 빌드/리사이즈 때만 재두고
 * (measure) 매 프레임에는 스크롤 값과 비교만 한다. 프레임마다 레이아웃을 읽는
 * 것은 마커 위치를 잡을 링크 두 개뿐이다.
 *
 * renderGrid 가 결과 영역을 다시 그릴 때마다 buildSectionNav 가 다시 호출되며
 * 캐시도 새로 잡힌다.
 *
